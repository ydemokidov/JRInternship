...
function processSearch(root, currentPage) {
    ...
    if (planet !== "") {
        sufix += sufix=="?"?"planet=" + planet:"&planet=" + planet;
    }
    if (shipType !== "Any") {
        sufix += sufix=="?"?"shipType=" + shipType.toUpperCase():"&shipType=" + shipType.toUpperCase();
    }
    if (yearAfter !== "") {
        sufix += sufix=="?"?"after=" + yearAfter:"&after=" + yearAfter;
    }
    if (yearBefore !== "") {
        sufix += sufix=="?"?"before=" + yearBefore:"&before=" + yearBefore;
    }
    if (isUsed !== null) {
        sufix += sufix=="?"?"isUsed=" + isUsed:"&isUsed=" + isUsed;
    }
    if (speedMin !== "") {
        sufix += sufix=="?"?"minSpeed=" + speedMin:"&minSpeed=" + speedMin;
    }
    if (speedMax !== "") {
        sufix += sufix=="?"?"maxSpeed=" + speedMax:"&maxSpeed=" + speedMax;
    }
    if (crewSizeMin !== "") {
        sufix += sufix=="?"?"minCrewSize=" + crewSizeMin:"&minCrewSize=" + crewSizeMin;
    }
    if (crewSizeMax !== "") {
        sufix += sufix=="?"?"maxCrewSize=" + crewSizeMax:"&maxCrewSize=" + crewSizeMax;
    }
    if (ratingMin !== "") {
        sufix += sufix=="?"?"minRating=" + ratingMin:"&minRating=" + ratingMin;
    }
    if (ratingMax !== "") {
        sufix += sufix=="?"?"maxRating=" + ratingMax:"&maxRating=" + ratingMax;
    }

    sufix += sufix=="?"?"pageNumber=" + (+currentPage - 1):"&pageNumber=" + (+currentPage - 1);
